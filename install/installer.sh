...
_svminstaller_install_package() {
	...
	unzip -j "$download_path" "svm-0.3.7/src/bin/*" -d "$install_path/bin" > /dev/null 2>&1
	unzip -j "$download_path" "svm-0.3.7/src/shims/*" -d "$install_path/shims" > /dev/null 2>&1
...
svm_package_url="https://github.com/scriptcs-contrib/svm/archive/v0.3.7.zip"
