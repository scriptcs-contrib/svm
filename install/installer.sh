...
_svminstaller_install_package() {
	...
	unzip -j "$download_path" "scriptcs-svm-0.3.2/src/bin/*" -d "$install_path/bin" > /dev/null 2>&1
	unzip -j "$download_path" "scriptcs-svm-0.3.2/src/shims/*" -d "$install_path/shims" > /dev/null 2>&1
...
svm_package_url="https://github.com/paulbouwer/scriptcs-svm/archive/v0.3.2.zip"
